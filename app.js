...
// Permanent EV stations layer
let stationsLayer = L.layerGroup().addTo(map);

// Temporary markers (source/destination)
...
// Function to mark source and destination
function markLocations(source, destination) {
    markersLayer.clearLayers(); // Only clear old src/dest markers
    L.marker(source).addTo(markersLayer).bindPopup("Source").openPopup();
    L.marker(destination).addTo(markersLayer).bindPopup("Destination").openPopup();
}

// Function to calculate route using OpenRouteService
function calculateRoute(source, destination, battery) {
    routeLayer.clearLayers();

    fetch(`https://api.openrouteservice.org/v2/directions/driving-car?api_key=${apiKey}&start=${source[1]},${source[0]}&end=${destination[1]},${destination[0]}`)
        .then(res => res.json())
        .then(data => {
            ...
            const batteryNeeded = (distance / 5) * 10; // 10% per 5 km
...
function findNearestStation(lat, lon, extraBatteryNeeded) {
    ...
    if (nearest) {
        L.marker([nearest.lat, nearest.lon], { icon: L.icon({
            ...
            iconSize: [30, 30]
        })}).addTo(markersLayer).bindPopup(`Nearest Charging Station: ${nearest.name}`).openPopup();
    }
}
...
document.getElementById('ev-form').addEventListener('submit', function (e) {
    ...
    if (!sourceCoords || !destinationCoords) {
        alert("⚠️ Please select both source and destination from the suggestions.");
        return;
    }

    sourceCoords = JSON.parse(sourceCoords); // [lon, lat]
    destinationCoords = JSON.parse(destinationCoords); // [lon, lat]
